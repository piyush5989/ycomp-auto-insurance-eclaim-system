 ...
 * Step 1: Policy number + vehicle registration
 * Step 2: Incident date, type, description
...
export default function SubmitClaimPage() {
  ...
  const onSubmit = (data: SubmitClaimFormData) => {
    submitClaim.mutate(data, {
      onSuccess: (response) => {
        navigate(`/customer/claims/${response.data?.claimId}`);
      }
    });
  };
  ...
  if (submitClaim.isSuccess) {
    return (
      <div className="card max-w-lg mx-auto text-center py-12">
        ...
        <p className="text-gray-500">Your claim ID: <code className="bg-gray-100 px-2 py-0.5 rounded text-sm">{submitClaim.data?.data?.claimId}</code></p>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {step === 0 && (
            <>
              ...
              <div>
                ...
                {errors.vehicleRegistration && <p className="text-red-500 text-xs mt-1">{errors.vehicleRegistration.message}</p>}
              </div>
            </>
          )}
          ...
          {step === 1 && (
            <>
              ...
              <div>
                ...
                <input {...register('incidentLocation')} placeholder="e.g. Highway 101, Exit 42" className="input" />
              </div>
              <div>
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" id="policeReport" {...register('policeReportFiled')}
                  className="w-4 h-4 rounded border-gray-300 text-primary-800" />
                <label htmlFor="policeReport" className="text-sm text-gray-700">Police report has been filed</label>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Policy Number</span><span className="font-medium">{formValues.policyNumber}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Vehicle</span><span className="font-medium">{formValues.vehicleRegistration}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Incident Date</span><span className="font-medium">{formValues.incidentDate}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Claim Type</span><span className="font-medium">{formValues.claimType?.replace('_', ' ')}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Police Report</span><span className="font-medium">{formValues.policeReportFiled ? 'Filed' : 'Not filed'}</span></div>
              </div>
              {submitClaim.isError && (
        ...
        <div className="flex justify-between mt-4">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button type="submit" disabled={submitClaim.isPending} className="btn-primary">
